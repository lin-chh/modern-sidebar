...
export const MenuItem = styled.div`
  ${p => !p.isSidebarOpen && `
    text-align: center;
    ${`background-color: ${p.colorPalette.selectedBackgroundCollapsedMode === 'dark' ? 'rgba(0, 0, 0, 0.6)' : 'rgba(255, 255, 255, 0.6)'}`};
  `};

  padding: 6px 20px;
  font-weight: 600;
  color: ${p => true ? p.colorPalette.fontColorSelected : p.colorPalette.fontColor} ;  
  font-family: ${p => p.font};
  white-space: nowrap;
  ...
  &:after {
    content: '';
    border: 1px solid p.colorPalette.fontColorSelected;
    display: 'block';
    margin: 8px 0 4px;    
    transition: .1s ease-in all;
  };
...
export const DropdownIcon = styled.span`
  position: absolute;
  top: ${p => true ? '16px' : '12px'};
  right: 24px;
  border: solid ${p => true ? p.colorPalette.fontColorSelected : p.colorPalette.fontColor};
  border-width: 0 1px 1px 0;
  padding: 3px;
  transform: ${p => true ? 'rotate(-135deg)' : 'rotate(45deg)'};
  transition: .4s;
`;
